...
- [ ] **Redux Slice Cleanup**: Audit slices for redundant state and ensure consistent normalization (especially for Timestamps).
...
- [ ] **Conflict Detection Visuals**: Show a "soft warning" (e.g., yellow dot) in the roster when a user is assigned to multiple positions/teams on the same day.
- [ ] **Mobile "Edit Mode" FAB**: Implement a floating action button to toggle between Read-Only (navigation) and Edit (drag/click assignments) modes on mobile.
- [ ] **Persistent Scroll Positioning**: Ensure that when navigating back from a Thought or Profile, the Roster Table returns to the same scroll position.
