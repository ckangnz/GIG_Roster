...
const GuestPage = () => {
  ...
  useEffect(() => {
    if (!inviteOrgId || !firebaseUser || !userData || autoJoinAttempted.current)
      return;
  ...
  const handleJoin = async (org: Organisation) => {
    ...
    try {
      ...
      if (result.membership.isApproved) {
        // Approved immediately (requireApproval: false) — go straight in
        dispatch(setActiveOrgId(org.id));
      } else {
        // Pending approval — go to select-org to see live approval status
      ...
      <div className={wizardStyles.wizardContainer}>
        ...
        {step !== 1 && (
          <div className={wizardStyles.wizardBody}>
            ...
            {step === 2 && inviteOrgId && <LoadingPage />}
