...
const SlotItem = ({
  ...
  return (
    <SortableItem
      value={slot}
      label={
        slot.label || t("management.team.newSlot", { defaultValue: "New Slot" })
      }
    >
      <div
        style={{
...
const EventItem = ({
  ...
  return (
    <SortableItem
      value={ev}
      label={
        ev.label || t("management.team.newEvent", { defaultValue: "New Event" })
      }
    >
      <div
        style={{
      ...
      >
        ...
        <div style={{ flex: "1.5 1 120px" }}>
          <SelectField
            ...
            value={ev.day}
            onChange={(e) => onUpdate(ev.id, "day", e.target.value as Weekday)}
          >
            {WEEK_DAYS.map((d) => (
...
const TeamConfigModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      const draftData = team
        ? JSON.parse(JSON.stringify(team))
        : { ...defaultTeam, id: crypto.randomUUID() };
  ...
  const handleAddEvent = () => {
    ...
    updateDraft({
      recurringEvents: [...(draft.recurringEvents || []), newEvent],
    });
  };
  ...
  const footer = (
    <div style={{ display: "flex", gap: "12px", width: "100%" }}>
      ...
      >
        {team
          ? t("management.team.applyChanges")
          : t("management.team.createTeam")}
      </Button>
    </div>
  );
  ...
  return (
    <Modal
      ...
      title={
        team
          ? t("management.team.editTitle", { name: team.name })
          : t("management.team.createTitle")
      }
      footer={footer}
    >
      <div
      ...
      >
        ...
        <SettingsGroup label={t("management.team.generalInfo")}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
            ...
            <input
              ...
              onChange={(e) =>
                updateDraft({ maxConflict: parseInt(e.target.value) || 1 })
              }
              style={{ width: "100px" }}
            />
            <p
        ...
        {/* Roster Mode Section */}
        <SettingsGroup
          label={t("management.team.rosterAccuracy")}
          description={t("management.team.rosterAccuracyDesc")}
        >
          <div className={localStyles.rosterModeContainer}>
            ...
            >
              ...
              <div style={{ textAlign: "left" }}>
                <div style={{ fontWeight: 700 }}>
                  {t("management.team.daily")}
                </div>
                <div style={{ fontSize: "0.75rem", opacity: 0.7 }}>
                  {t("management.team.dailyDesc")}
                </div>
              </div>
            </button>
            <button
            ...
            >
              ...
              <div style={{ textAlign: "left" }}>
                <div style={{ fontWeight: 700 }}>
                  {t("management.team.slotted")} ({t("onboarding.comingSoon")})
                </div>
                <div style={{ fontSize: "0.75rem", opacity: 0.7 }}>
                  {t("management.team.slottedDesc")}
                </div>
              </div>
            </button>
          </div>
        ...
        {draft.rosterMode === "slotted" && (
          <SettingsGroup
            label={t("management.team.timeSlotsConfig")}
            description={t("management.team.timeSlotsDesc")}
          >
              ...
              {(draft.slots || []).map((slot) => (
                <SlotItem
                  ...
                  onUpdate={(id, field, val) => {
                    const updated = (draft.slots || []).map((s) =>
                      s.id === id ? { ...s, [field]: val } : s,
                    );
                    updateDraft({ slots: updated });
                  }}
                  onRemove={(id) => {
                    const updated = (draft.slots || []).filter(
                      (s) => s.id !== id,
                    );
                    updateDraft({ slots: updated });
                  }}
            ...
            >
              ...
              {t("management.team.addCustomSlot")}
            </Button>
          </SettingsGroup>
        )}
        ...
        {/* Positions */}
        <SettingsGroup
          label={t("management.team.allowedPositions")}
          description={t("management.team.allowedPositionsDesc")}
        >
        ...
        {/* Preferred Days */}
        <SettingsGroup
          label={t("management.team.preferredDays")}
          description={t("management.team.preferredDaysDesc")}
        >
            ...
            {WEEK_DAYS.map((day) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span className={localStyles.dayLabel}>
        ...
        {/* Calendar Setup */}
        <SettingsGroup
          label={t("management.team.calendarSetup")}
          description={t("management.team.calendarSetupDesc")}
        >
          <div
          ...
          >
            {(draft.recurringEvents || []).map((ev) => (
              <EventItem
                ...
                ev={ev}
                onUpdate={(id, field, val) => {
                  const updated = (draft.recurringEvents || []).map((e) =>
                    e.id === id ? { ...e, [field]: val } : e,
                  );
                  updateDraft({ recurringEvents: updated });
                }}
                onRemove={(id) => {
                  const updated = (draft.recurringEvents || []).filter(
                    (e) => e.id !== id,
                  );
                  updateDraft({ recurringEvents: updated });
                }}
        ...
        >
          ...
          >
            <span style={{ fontSize: "0.95rem", fontWeight: 500 }}>
              {t("management.team.allowAbsence")}
            </span>
            <Toggle
